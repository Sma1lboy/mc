# 06 · UI 布局融合 · Modrinth 骨架 + PCL 主题引擎

> 用户同时喜欢 Modrinth App 的**布局/信息架构**和 PCL 的**主题色引擎/质感**。本文把两者融合成一个统一方向。
> 本文的**布局**部分覆盖 [05](./05-ui-design-pcl.md) 的 §2 布局;[05] 的**主题 token / 可调主题色 / 动画**继续有效(并在此扩成深色)。
...
**一句话**:**Modrinth 的骨,PCL 的皮**。布局抄 Modrinth,换肤系统用 PCL。
...
:root, [data-theme="dark"] {
  ...
  /* accent 色阶:由 H/S/L 生成(PCL 引擎),默认 Modrinth 绿 ≈ h150 */
...
| 强调色 | 可调蓝 | 固定绿 | **可调 accent(默认绿),保留 PCL 引擎** |

> 保留下来的 PCL 灵魂:**可调主题色引擎 + 数字色阶 token + 动画质感 + 左下 Toast**。
> 换成 Modrinth 的:**三区布局 + 图标栏导航 + 深色 + Home dashboard + 更圆的卡片**。
...
> 原则不变:**先把 token 引擎 + 三区骨架跑通**,再填页面。换肤(深浅 + accent)零成本是 PCL 引擎给的底子。
