...
## 已落地 · 玻璃 UI 一致性 pass(2026-06)

一轮「让界面不再各写各的」的收口,要点:

- **危险色令牌**:删除/关闭/错误等破坏性 UI 统一走 `danger` 令牌(`tokens.css` 定义 `--danger`/`-hover`/
  `-text`/`-soft`,`tailwind.css` 桥接;深浅各给可读兜底),取代此前散落的 `#e5848a`/`#d9534f`/`#ff5c5c`/
  `#e5484d` 等十余处硬编码红。详见 [[danger 令牌]] 约定。
- **空状态组件**:`components/EmptyState.tsx` 一处定义(实线玻璃卡 + 居中文案 + 可选 CTA,`compact` 给窄栏),
  Home / Library / ContextBar(好友/动态)/ 搜索结果统一改用,去掉了虚线占位那种「没做完」感。
- **TopBar**:左侧不再重复页面标题(各页自带 H1 即唯一标题);运行状态收进玻璃药丸;品牌 `kobeMC` 落右上角灰字;
  移除与各页自身返回冲突的占位前进/后退箭头。
- **返回一致性**:详情页(实例 / Modpack / 项目)统一「← 返回」文字按钮 + Esc 返回(灯箱/弹窗/输入态不抢 Esc)。
- **实例「添加内容」流**:Mods/资源包/光影/数据包默认显示「已安装」,点 `+ 添加` 进入**复用探索页**的全屏浏览
  (隐藏头部+标签条),行内「下载」后台并行安装(不阻塞其它行)+「已添加」即时反馈,点进可挑版本。

> IPC 类型/命令改由 Rust 生成(见 [ipc-bindings.md](./ipc-bindings.md)),与本页 UI 工作同批落地。
