...
import { activeRoot, refreshInstances, isKobeSignedIn, setCurrentPage } from "../store";
import { t } from "../i18n";
 ...
 */
export const JoinRealmDialog: Component<{
  open: boolean;
  onClose: () => void;
    ...
    <Dialog open={props.open} onClose={props.onClose} label={t("realm.joinTitle")}>
      <div class="p-[20px] flex flex-col gap-[14px]">
        ...
        <Show when={!isKobeSignedIn()}>
          <p class="text-[12px] text-danger-text">
            {t("realm.needLogin")}{" "}
            <button
              class="text-accent hover:underline bg-transparent border-none cursor-pointer p-0"
              onClick={() => {
                props.onClose();
        ...
        <label class="flex flex-col gap-[6px]">
          ...
          <input
            class="h-[38px] px-[14px] rounded-none text-[13px] text-fg bg-sidebar shadow-input w-full font-mono tracking-[0.16em] uppercase placeholder:text-faint focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
            type="text"
            maxLength={6}
          ...
          />
          <span class="text-[11px] text-faint">{t("realm.joinHint")}</span>
        </label>
