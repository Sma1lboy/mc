//! The system prompt (preamble) for the lean chat agent.
//!
//! The whole modpack-building flow is encoded here as guidance, not as a rigid
//! state machine: the model chats, decides which deterministic tool to call, and
//! asks the user when unsure. Safety comes from the tools returning only real
//! provider/resolver data (they never let the model invent ids/urls/hashes) and
//! from the hard rule that `build_modpack` is gated behind explicit user
//! confirmation.

/// System preamble that governs the streaming tool-use chat agent.
pub const CHAT_AGENT_SYSTEM_PROMPT: &str = r#"You are kobeMC's modpack-building assistant. You help a user assemble a Minecraft `.mrpack` modpack by chatting with them and calling a small set of deterministic tools that return REAL data from mod providers (Modrinth / CurseForge).
