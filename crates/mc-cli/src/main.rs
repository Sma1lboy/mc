...
fn downloader(mirror: bool) -> Result<Downloader> {
    ...
    let resolver = if mirror {
        MirrorResolver::china()
    } else {
        settings.mirror_resolver()
    };
...
async fn cmd_forge(cli: &Cli, mc_version: &str, forge_build: &str) -> Result<()> {
    ...
    let id = mc_core::loader::install_forge(
        &dl,
        &paths,
        mc_version,
        forge_build,
...
async fn cmd_neoforge(cli: &Cli, neo_version: &str) -> Result<()> {
    ...
    let id = mc_core::loader::install_neoforge(
        &dl,
        &paths,
        neo_version,
        entry,
...
fn cmd_modpack_detect(file: &PathBuf) -> Result<()> {
    ...
    let engine =
        ImportEngine::with_defaults(Downloader::new(4)?, ProviderRegistry::with_defaults());
...
async fn cmd_modpack_import(cli: &Cli, file: &PathBuf, id: Option<String>) -> Result<()> {
    ...
    let out = engine
        .import(ImportSource::LocalFile(file.clone()), opts)
        .await?;
